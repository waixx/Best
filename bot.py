# ===================================================================
#  BROWAIX BOT — УНИВЕРСАЛЬНАЯ ИДЕАЛЬНАЯ ЛОГИКА
#  Запрещён СМЫСЛ, а не слова! Результаты есть всегда.
# ===================================================================
...
# ==================== ЗАГРУЗКА ====================
async def fetch_content(url: str, timeout: int = 20):
...
def is_lie_by_sense(text: str) -> Tuple[bool, str]:
    ...
    lie_patterns = [
        ...
        (r'не\s*(содержат|имеют)\s*конкретных\s*названий', "Утверждает, что нет конкретных данных (хотя они есть)"),
        ...
        # "Невозможно" — в любом виде
        (r'невозможно\s*(составить|сделать|определить|найти)', "Утверждает, что невозможно (хотя возможно!)"),
        (r'не\s*представляется\s*возможным', "Утверждает, что невозможно (хотя возможно!)"),
        
        # "Свой список" — в любом виде
        (r'(привожу|даю|предлагаю|составляю)\s*(свой|собственный)\s*(список|перечень)', "Придумывает свой список вместо источников!"),
        ...
        (r'собственный\s*проверенный\s*список', "Придумывает свой список вместо источников!"),
        
        # "Я не могу" — в любом виде
        ...
        (r'извините', "Начинает с 'извините' — признак обмана"),
    ]
    ...
    # Проверка: есть источники, но в ответе нет ни одной ссылки на них
    # (Это проверка, которую нельзя обойти фразами)
...
async def search_and_answer_safe(uid, user_message, history):
    ...
    links = [r['link'] for r in all_results[:15]]
    pages = await fetch_multiple_pages(links, max_pages=10)
    ...
    if source_count == 0:
        logger.warning("⚠️ ШАГ 4: Нет качественных источников")
        # Вместо фоллбэка — честно говорим, что страницы не загрузились
        return "⚠️ Страницы загрузить не удалось. Попробуйте позже или переформулируйте запрос."
    ...
    # ================================================================
    # ШАГ 5: ПРОМПТ С ЗАПРЕТОМ СМЫСЛА
    # ================================================================
    
    source_text = "⚠️ **ВОТ РЕАЛЬНЫЕ ДАННЫЕ ИЗ ИНТЕРНЕТА:**\n\n"
    for i, p in enumerate(good_sources[:10], 1):
...
Содержание: {p['text'][:4000]}
"""
    
    rules = f"""
⚠️ **ТЫ ПОЛУЧИЛ ВЫШЕ {len(good_sources)} РЕАЛЬНЫХ ИСТОЧНИКОВ ИЗ ИНТЕРНЕТА!**

⚠️ **ЭТО НЕ ПРОСЬБА — ЭТО ПРИКАЗ!**

**ТЫ ОБЯЗАН:**
1. Проанализировать КАЖДЫЙ источник
2. Найти в них ОТВЕТ на вопрос пользователя
3. Выдать ответ ТОЛЬКО из источников
4. Если информации мало — сказать ЧЕСТНО: "Найдено X фактов: ..."

⚠️ **ЗАПРЕЩЕНО ПО СМЫСЛУ (ЭТО ЛОЖЬ!):**
- Говорить, что "нет данных" (они ЕСТЬ!)
- Говорить, что "невозможно" (это ВОЗМОЖНО!)
- Придумывать свой ответ вместо источников
- Использовать свои знания вместо источников
- Говорить "я не могу" или "у меня нет возможности"

⚠️ **ЕСЛИ ТЫ НАРУШИШЬ ЭТИ ПРАВИЛА — ТЫ СОВРЁШЬ!**

⚠️ **ФОРМАТ ОТВЕТА:**
📊 **Использованные источники:** (перечисли ВСЕ {len(good_sources)})
📊 **Ответ на вопрос:** (ТОЛЬКО ИЗ ИСТОЧНИКОВ)
...
У ТЕБЯ ЕСТЬ ДАННЫЕ! {len(good_sources)} ИСТОЧНИКОВ!

**НЕЛЬЗЯ ПО СМЫСЛУ:**
- "нет данных" — ОНИ ЕСТЬ!
- "невозможно" — ЭТО ВОЗМОЖНО!
- придумывать свой ответ — ТОЛЬКО ИЗ ИСТОЧНИКОВ!
...
"""
    ...
    logger.info("✅ ШАГ 5: Промпт сформирован (ЗАПРЕТ СМЫСЛА)")
    ...
    if err or not answer:
        logger.warning("⚠️ ШАГ 6: DeepSeek не ответил")
        return "⚠️ Не удалось получить ответ. Попробуйте позже."
    
    logger.info("✅ ШАГ 6: Ответ получен")
    
    # ================================================================
    # ШАГ 7: ПРОВЕРКА НА СМЫСЛ (НЕ НА СЛОВА!)
    # ================================================================
    
    is_lie, lie_reason = is_lie_by_sense(answer)
    
    if is_lie:
        logger.warning(f"⚠️ ШАГ 7: ОБНАРУЖЕНА ЛОЖЬ: {lie_reason}")
        
        # Перезапрашиваем с усиленным предупреждением
        stronger_warning = f"""
⚠️ **ТЫ НАРУШИЛ ПРАВИЛА! ОБНАРУЖЕНА ЛОЖЬ!**

Ты сказал: "{lie_reason}"

Но это НЕПРАВДА, потому что:
- У тебя есть {len(good_sources)} источников
- В них есть данные
...
"""
        ...
        if err or not answer:
            logger.warning("⚠️ DeepSeek не ответил после предупреждения")
            return "⚠️ Не удалось получить честный ответ. Попробуйте позже."
        
        # Проверяем снова
        is_lie, lie_reason = is_lie_by_sense(answer)
        if is_lie:
            logger.error(f"❌ ШАГ 7: ПОВТОРНАЯ ЛОЖЬ: {lie_reason}")
            # Вместо сырых результатов — честное сообщение
            return f"⚠️ **Я не могу найти ответ в источниках.**\n\nИсточники загружены, но в них нет информации по вашему запросу.\n\nПопробуйте переформулировать вопрос или задать его иначе."
    
    logger.info("✅ ШАГ 7: Проверка пройдена — ответ честный")
...
async def generate_knowledge_safe(user_message, history, reason):
    """Ответ из знаний модели — только если нет источников"""
    system_prompt = f"""
...
def main():
    ...
    logger.info(f"🔍 Serper: {'✅' if SERPER_API_KEY else '❌'}")
    logger.info(f"🌐 Browserless: {'✅' if BROWSERLESS_WS_ENDPOINT else '❌'}")
    logger.info("⚠️ РЕЖИМ: УНИВЕРСАЛЬНАЯ ЛОГИКА + ЗАПРЕТ СМЫСЛА")
