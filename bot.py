...
#  Все лазейки перекрыты + Универсальный парсинг + Оптимизация
# ===================================================================
...
async def search_and_answer_safe(uid, user_message, history):
    ...
    # ================================================================
    # ШАГ 3: ПРОМПТ — ЗАПРЕТ ВСЕХ ЛАЗЕЕК
    # ================================================================
...
4. Если данных много — показать ВСЁ
5. Если данных мало — показать ЧТО ЕСТЬ
...
"""
    ...
    logger.info("✅ Промпт сформирован (запрет ВСЕХ лазеек)")
...
def main():
    ...
    logger.info(f"🔍 Serper: {'✅' if SERPER_API_KEY else '❌'}")
    logger.info(f"🌐 Browserless: {'✅' if BROWSERLESS_WS_ENDPOINT else '❌'}")
    logger.info("⚠️ РЕЖИМ: ЗАПРЕТ ВСЕХ ЛАЗЕЕК + УНИВЕРСАЛЬНЫЙ")
